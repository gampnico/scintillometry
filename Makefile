...
.PHONY:	install
install:	--install-deps	## Install base package with conda
		@echo "\nInstalling editable..."
		@pip install -e .

install-tests:	--install-deps	## Install with dependencies for tests
		@echo "\nInstalling dependencies (tests)..."
		@mamba install "pytest>=7.0" "pytest-dependency>=0.5" "coverage>=7.1" \
		-c conda-forge -y
...
install-docs:	--install-deps	## Install with local documentation
		...
		@mamba install sphinx "sphinx-rtd-theme>=1.1" -c conda-forge -y
...
--install-deps:	--check-python	# Private: install core dependencies only
